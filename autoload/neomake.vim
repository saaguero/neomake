    ...
    try
        ...
        if neomake#has_async_support()
            if has('nvim')
                ...
                let jobinfo.id = ch_info(job_channel)['id']
...
function! s:Make(options, ...) abort
    ...
    for name in enabled_makers
        ...
        if has_key(s:jobs_by_maker, maker_key)
            ...
            try
                call neomake#utils#DebugMessage('Stopping job: ' . jobinfo.id)
                if has('nvim')
                    call jobstop(jobinfo.id)
                else
                    call job_stop(jobinfo.job)
                endif
            catch /^Vim\%((\a\+)\)\=:E900/
...
function! s:accumulate_job_output(id, line) abort
  call add(s:jobs[a:id].data, a:line)
endfunction

function! neomake#MakeHandlerVim(channel, line) abort
    call s:accumulate_job_output(ch_info(a:channel)['id'], a:line)
endfunction
...
function! neomake#MakeHandlerVimClose(channel) abort
    ...
    let jobinfo = s:jobs[id]
    call neomake#MakeHandler(id, jobinfo.data, 'stdout')
    call neomake#MakeHandler(id, job_info(jobinfo.job)['exitval'], 'exit')
endfunction
...
function! neomake#MakeHandler(job_id, data, event_type) abort
    ...
    elseif a:event_type ==# 'exit'
        ...
        if has('nvim') && has_key(jobinfo, 'lines')
            if jobinfo.lines[-1] ==# ''
